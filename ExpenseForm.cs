    ...
    public partial class ExpenseForm : Form
    {
        ...
        private void InitializeComponent()
        {
            ...
            dgvExpense = new DataGridView
            {
                ...
                AlternatingRowsDefaultCellStyle = new DataGridViewCellStyle { BackColor = Color.FromArgb(255, 248, 248) },
        ...
        private void LoadData(DateTime? from = null, DateTime? to = null)
        {
            try
            {
                dgvExpense.DataSource = DatabaseHelper.GetRecords("Расход", from, to);
                dgvExpense.Columns["ID"].Visible = false;
                dgvExpense.Columns["Дата"].HeaderText = "Дата";
                dgvExpense.Columns["Товар"].HeaderText = "Товар";
                dgvExpense.Columns["Количество"].HeaderText = "Количество";
                dgvExpense.Columns["Дата"].Width = 100;
                dgvExpense.Columns["Количество"].Width = 100;
            }
            catch (Exception ex)
            {
                MessageBox.Show($"Ошибка загрузки расхода: {ex.Message}", "Ошибка", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
        }
        ...
        private void AddRecord(object sender, EventArgs e)
            {
                var form = new RecordForm("Расход");
                form.Owner = this;
        ...
        private void EditRecord(object sender, EventArgs e)
            {
                ...
                int id = (int)dgvExpense.CurrentRow.Cells["ID"].Value;
                string date = dgvExpense.CurrentRow.Cells["Дата"].Value.ToString();
                string product = dgvExpense.CurrentRow.Cells["Товар"].Value.ToString();
                int quantity = (int)dgvExpense.CurrentRow.Cells["Количество"].Value;

                var form = new RecordForm("Расход", id, date, product, quantity);
                form.Owner = this;
                if (form.ShowDialog() == DialogResult.OK)
                    LoadData();
            }
        ...
        private void DeleteRecord(object sender, EventArgs e)
        {
            ...
            if (MessageBox.Show("Удалить запись расхода?", "Подтверждение", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
            {
                DatabaseHelper.DeleteRecord("Расход", id);
                LoadData();
            }
        }
    }
